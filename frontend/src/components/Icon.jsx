// Icon.jsx — inline stroke-based SVG icon set (ported from the mockup
// src/icons.jsx), exposed as a component map plus a status glyph helper.
// Pure presentational; safe to use in server or client components.

function Svg({ children, size = 16, sw = 1.6, ...rest }) {
  return (
...
export const Icon = {
  ...
  diff: (p) => <Svg {...p}><path d="M12 3v18M5 8h14M5 16h14"/></Svg>,
  alert: (p) => <Svg {...p}><path d="M12 3l9 16H3z"/><path d="M12 10v4M12 17h.01"/></Svg>,
  ban: (p) => <Svg {...p}><circle cx="12" cy="12" r="8.5"/><path d="M6 6l12 12"/></Svg>,
  users: (p) => <Svg {...p}><circle cx="9" cy="8" r="3.2"/><path d="M3.5 19c0-3 2.4-5 5.5-5s5.5 2 5.5 5"/><path d="M15.5 5.4a3.2 3.2 0 010 5.9M17.5 14.3c1.8.7 3 2.2 3 4.7"/></Svg>,
  chart: (p) => <Svg {...p}><path d="M4 20V10M10 20V4M16 20v-8M21 20H3"/></Svg>,
  inspect: (p) => <Svg {...p}><path d="M6 3h8l4 4v14H6z"/><path d="M14 3v4h4"/><path d="M9.5 14.5l2 2 3.5-3.5"/></Svg>,
};

export function StatusGlyph({ status, size = 16 }) {
  ...
  return <Icon.dot size={size} />; // running / pending / idle
}
